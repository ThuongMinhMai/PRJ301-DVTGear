...
public class ProductDAO {
  ...
  public Product getProductDetail(int id) {
    ...
    String query = " SELECT p.productId, p.name, c.name AS category, b.name AS brand, p.description, p.images, p.price, p.categoryId, p.brandId, p.storage " + "FROM Product p " + "INNER JOIN Brand b ON p.brandId = b.brandId " + "INNER JOIN Category c ON p.categoryId = c.categoryId " + "WHERE p.productId = ?";
  ...
  public void createProduct(JSONObject jsonObject) {
    String query = "INSERT INTO Product ([name], categoryId, brandId, [description], price, images, storage) " + "VALUES (?, ?, ?, ?, ?, ?, ?)";
